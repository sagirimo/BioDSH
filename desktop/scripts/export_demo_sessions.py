...
--src-base 是录制这些会话时的工作目录根（默认取环境变量 BIODSH_DEMO_SRC 或 ~/BioDSH/demos），
demo 目录名会拼在它后面，并按 dsh 的规则编码成 session slug。"""
import argparse, json, os, re, shutil, subprocess, sys, uuid
CURL = '/mnt/c/Windows/System32/curl.exe'
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

def dsh_slug(cwd):
    # dsh 把 cwd 编成 sessions 子目录名：非字母数字全换成 '-'，首尾包一对 '--'
...
def main():
    ...
    ap.add_argument('--src-base', default=os.environ.get('BIODSH_DEMO_SRC', os.path.join(os.path.expanduser('~'), 'BioDSH', 'demos')))
    a = ap.parse_args(); plan = json.load(open(a.plan, encoding='utf-8'))
    src_base = a.src_base.rstrip('/\\')
    for demo, sessions in plan.items():
        ...
        src_cwd = f'{src_base}{os.sep}{demo}'
        slug = dsh_slug(src_cwd)
        out = []
        for s in sessions:
            ...
            out.append({'id': s['id'], 'title': s['title'], 'sourceCwd': src_cwd, 'file': f'.session/{s["id"]}.jsonl.zstd'})
